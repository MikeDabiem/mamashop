...
<?php } else { ?>
    ...
    <div class="cart-menu__footer">
        ...
        $min_order_price = get_field('min_price', 'options');
        $free_ship = get_field('free_ship_price', 'options');
        ?>
        <p class="cart-menu__footer--error error-min font-11-13 fw-400 transition-default <?php $cart_total < $min_order_price ? print 'show' : null; ?>">Мінімальна вартість замовлення складає <?= $min_order_price ?> грн</p>
        <p class="cart-menu__footer--error error-free font-11-13 fw-400 <?php $cart_total >= $min_order_price ? print 'show' : null; ?>">При замовленні на суму більше <?= $free_ship ?> грн доставка безкоштовна</p>
        <a href="<?= wc_get_checkout_url(); ?>" class="cart-menu__order std-btn font-15-18 fw-600 transition-default d-flex <?php $cart_total < $min_order_price ? print 'disabled-check' : null; ?>">
